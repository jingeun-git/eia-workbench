...
# 한글 [쪽 > 감추기] 대화상자의 전 항목 — 머리말·꼬리말·쪽번호·바탕쪽·테두리·배경·빈 줄.
# 사용자 확정(2026-07-21): 부분이 아니라 **전체 감추기**로 동작한다.
...
def _hide_page(hwp, phys: int, fields=HIDE_FIELDS):
    """지정한 **물리 쪽**에 감추기를 적용한다.

    ⚠ 과거에 "임의 쪽 감추기는 COM으로 불가(Execute=True인데 무동작)"라고 결론냈으나,
      그때 사용한 CreateAction()+CreateSet()+SetItem() 패턴은 **값이 파라미터셋에
      반영되지 않는다**는 것이 나중에 _set_number에서 확인됐다(2026-07-20).
      즉 그 결론은 무효이고, 여기서는 검증된 HParameterSet 속성 대입으로 다시 시도한다.
      실제로 걸렸는지는 호출부가 pghd 개수 증감으로 확인한다 — 반환값을 믿지 않는다.
    """
    _goto_page(hwp, phys)
    act = hwp.HAction
