...
export async function init(section, { bridge, toast, V }) {
  ...
  const { decodeTextBytes } = await loadShared("textenc.js", V);
  ...
  function drawMarkers() {
    ...
    for (const r of rows) {
      ...
      m.on("click", () => selectRow(r.id, false));
      markers.set(r.id, m);
  ...
  $("#gc-file").addEventListener("change", async (ev) => {
    ...
    if (!f) return;
    try {
      const { list: table, headerFrom } = await readTable(f);
      ...
      if (headerFrom < 0) log("머리글이 없어 열 이름을 A열·B열…로 붙였습니다 — 모든 행을 데이터로 읽습니다", "warn");
      else if (headerFrom > 0) log(`${headerFrom + 1}행을 머리글로 인식했습니다(위 ${headerFrom}행은 제목·빈행으로 보고 제외)`, "warn");
      showMapping(table);
    } catch (e) {
      log(`✗ ${e.message}`, "fail");
  ...
  async function readTable(file) {
    ...
    let wb;
    if (isCsv) {
      wb = window.XLSX.read(decodeTextBytes(new Uint8Array(buf)), { type: "string" });
    } else {
    ...
    const kept = aoa.map((r, i) => ({ r, i })).filter(({ r }) => r.some((c) => String(c ?? "").trim() !== ""));
    const rows = kept.map((k) => k.r);
    if (!rows.length) return { list: [], headerFrom: -1 };

    const width = Math.max(...rows.map((r) => r.length));
    const filled = (r) => r.filter((c) => String(c ?? "").trim() !== "").length;

    let hi = 0;
    for (let i = 1; i < Math.min(rows.length, 10); i++) if (filled(rows[i]) > filled(rows[hi])) hi = i;

    const looksData = (c) => {
      const s = String(c ?? "").trim();
      return s !== "" && (/^-?\d+(\.\d+)?$/.test(s) || s.length > 12 || /(읍|면|동|리|로|길)\s*\S*\d/.test(s));
    };
    const isHeader = filled(rows[hi]) > 0 && !rows[hi].some(looksData);
    const header = isHeader
      ? rows[hi].map((c, i) => String(c ?? "").trim() || `${String.fromCharCode(65 + i)}열`)
      : Array.from({ length: width }, (_, i) => `${String.fromCharCode(65 + i)}열`);
    const body = isHeader ? rows.slice(hi + 1) : rows;
    const list = body.map((r) => {
      const o = {};
      header.forEach((h, i) => { o[h] = String(r[i] ?? "").trim(); });
      return o;
    });
    return { list, headerFrom: isHeader ? kept[hi].i : -1 };
  }
