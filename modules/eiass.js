    ...
    <div data-epane="seq">
      ...
      <div style="display:flex;gap:var(--space-3);align-items:center;flex-wrap:wrap">
        ...
        <label style="font-size:var(--text-sm);display:flex;align-items:center;gap:6px">
          <input type="checkbox" id="es-zip"> ZIP으로 묶기 <span style="color:var(--text-dim)">(로컬 런처로 저장할 때만 — 저장 폴더 지정 필요)</span>
        </label>
    ...
    try {
      if (viaBridge) {
        ...
        if ($("#es-zip").checked) {
          ...
          esLog("※ ZIP으로 묶기는 로컬 런처가 저장할 때만 됩니다 — 저장 폴더가 지정되지 않아 개별 파일로 받습니다.", "warn");
          toast("ZIP으로 묶기는 저장 폴더를 지정해야 적용됩니다 — 이번에는 개별 파일로 받습니다", "warn");
        }
        if (bridge.state === "ok")
