...
import { readWorkbook } from "../shared/textenc.js";
...
async function fetchParcelsInBox(bounds, vkey, log, checkCancel) {
  ...
  const nx = Math.ceil(kx / 3), ny = Math.ceil(ky / 3);
  const tiles = nx * ny;
  if (tiles > 120)
...
async function readShpFiles(files, log) {
  ...
  if (byExt.zip) {
    ...
    const geoms = shp.parseShp(await byExt.shp.arrayBuffer());
    let props = null;
    if (byExt.dbf) {
...
function addressScore(v) {
  const s = String(v ?? "").trim();
  if (s.length < 6) return 0;
  let sc = 0;
  if (/(특별시|광역시|특별자치시|특별자치도|[가-힣]{2,}(도|시|군|구))(\s|$)/.test(s)) sc += 2;
  if (/[가-힣]+(읍|면|동|리|로|길|가)(\s|$)/.test(s)) sc += 2;
  if (/(산\s*)?\d+(-\d+)?\s*$/.test(s)) sc += 1;
  return sc;
}
...
async function readAddressesFromExcel(file) {
  ...
  if (!aoa.length) return { list: [], colIndex: 0, header: "", picked: false };

  const width = Math.max(...aoa.map((r) => r.length), 1);
  const headerRow = aoa[0].map((c) => String(c ?? "").trim());
  const hasHeader = headerRow.some((h) => HEADER_HINTS.some((k) => h.toLowerCase().includes(k)));
  ...
  for (let c = 0; c < width; c++) {
    ...
    let score = vals.reduce((n, v) => n + addressScore(v), 0) / vals.length;

    if (HEADER_HINTS.some((k) => (headerRow[c] || "").toLowerCase().includes(k))) score += 1.5;
    if (score > best.score) best = { idx: c, score };
  }
  const list = body.map((r) => String(r[best.idx] ?? "").trim()).filter(Boolean);


  if (!hasHeader && list.length > 1 && addressScore(list[0]) === 0
      && list.slice(1).some((v) => addressScore(v) > 0)) list.shift();
  return { list, colIndex: best.idx, header: headerRow[best.idx] || "", picked: best.score > 0 };
}
...
export function init(section, { toast, bridge }) {
  ...
  <div class="panel">
    ...
    <div data-pane="addr" style="display:none">
      <div class="field">
        ...
        <p class="help">지번·도로명 모두 가능. 파일(엑셀·CSV)에서 불러오면 주소가 담긴 열을 자동으로 찾습니다 → <button type="button" class="btn btn-secondary" id="pc-xlsx-btn" style="height:28px;padding:0 10px;font-size:12px">엑셀·CSV 불러오기</button>
        <input type="file" id="pc-xlsx" accept=".xlsx,.xls,.csv" style="display:none"></p>
      </div>
    </div>
  ...
  $("#pc-xlsx").addEventListener("change", async (e) => {
    ...
    if (!f) return;
    try {
      const { list, colIndex, header, picked } = await readAddressesFromExcel(f);
      $("#pc-addrs").value = list.join("\n");
      const colName = String.fromCharCode(65 + colIndex) + (header ? `열 "${header}"` : "열");
      if (!list.length) {
        toast("주소를 찾지 못했습니다 — 지번주소가 담긴 열이 있는지 확인해주세요", "fail");
      } else if (!picked) {


        toast(`${list.length}건을 ${colName}에서 불러왔지만 지번주소 형태가 아닙니다 — 목록을 확인해주세요`, "warn");
      } else if (colIndex > 0) {

        toast(`주소 ${list.length}건을 불러왔습니다 · 주소 열 = ${colName}`, "ok");
      } else {
        toast(`주소 ${list.length}건을 불러왔습니다`, "ok");
      }
